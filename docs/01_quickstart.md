...
これにより、`weights/` ディレクトリに SD 1.5 のトークナイザと miniSD のモデルがダウンロードされます。miniSD は SD 1.4 を 256×256 向けにチューニングしたモデルで、CPU でも実用的な速度で画像を生成できます。
...
```bash
uv run my-sd15 -p "a cat sitting on a windowsill" --seed 123 -o sd15.png
```
