...
テキストをトークン化し、CLIP Text Encoder で条件ベクトルに変換します。「a cat sitting on a windowsill」は 8 トークンに分割され、`<|startoftext|>` と `<|endoftext|>` を加えた 10 トークンの後ろは `<|endoftext|>` (ID: 49407) で 77 まで埋められます。`'Hello World'` と `'hello world'` が同一のトークン列になることから、小文字化が機能していることが確認できます。
