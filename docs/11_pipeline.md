ページ：[01](01_quickstart.md) | [02](02_overview.md) | [03](03_clip.md) | [04](04_conv2d.md) | [05](05_groupnorm.md) | [06](06_resblock.md) | [07](07_unet.md) | [08](08_cross_attention.md) | [09](09_ddim.md) | [10](10_vae.md) | **11** | [12](12_lora.md) | [13](13_architecture.md)
...
- $s = 0.0$: 無条件ノイズ予測のみ使用（プロンプトは無視される）
...
$s = 0.0$ のときは式全体が $\epsilon_{uncond}$ になるため、プロンプトは完全に無視されます。$s = 1.0$ のときは式全体が $\epsilon_{cond}$ になるため、無条件側の予測は結果に影響しません。つまり Negative Prompt が効くのは $s > 1.0$ のときだけです。また $s = 1.0$ では無条件側の U-Net 呼び出しが不要になるため、ステップあたりの計算量が半分になります。
...
つまり Negative Prompt は特別な仕組みではなく、CFG の「無条件」側にテキストを入れるだけです。前述のとおり、これが意味を持つのは `cfg_scale > 1.0` のときに限られます。
...
パイプラインの各ステップの中間テンソルや CFG スケールの効果など、本文中の数値を確認するためのスクリプトです。
